...
TreeNode1* mybuildTree(vector<int> preorder, vector<int> inorder, int pre_left, int pre_right, int in_left, int in_right) {
    ...
    //把根节点建立起来
    TreeNode1* now = new TreeNode1(preorder[preorder_root]);
...
TreeNode1* buildTree(vector<int>& preorder, vector<int>& inorder) {
    int n = preorder.size();
